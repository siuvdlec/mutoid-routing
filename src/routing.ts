...
import { fromEither, fromNullable, isNone, none, Option, option, some } from 'fp-ts/Option'
import { filter, isEmpty } from 'fp-ts/Record'
import { identity, tuple, pipe } from 'fp-ts/function'
...
export class Route {
     ...
     * @since 0.4.0
     */
    static parse(s: string, decode = true): Route {
 ...
 * @since 0.4.0
 */
export function format<A>(formatter: Formatter<A>, a: A, encode = true): string {
...
export function rgx<K extends string, T>(
    k: K,
    regex: RegExp,
    rgxParser: (p: string) => T,
    rgxFormatter: (t: T) => string
): Match<{ [_ in K]: T }> {
    return new Match(
        new Parser(r => {
            if (r.parts.length === 0) {
                ...
                const head = r.parts[0]
                const tail = r.parts.slice(1)
                return typeof head === 'string' && head.match(regex) !== null
                    ? some(tuple(singleton(k, rgxParser(head)), new Route(tail, r.query)))
                    : none
            }
        }),
        new Formatter((r, o) => new Route(r.parts.concat(rgxFormatter(o[k])), r.query))
    )
}
